#include "draw_tree.h"  // W tym pliku włączamy draw_tree.h
...
void drawTreeAndSave(Node* root) {
    int width = 1000;   // Zwiększamy szerokość obrazu
    int height = 1000;  // Zwiększamy wysokość obrazu
    ...
    if (root != nullptr) {
        ...
        double rootY = 100;
        double radius = 30;  // Zmniejszamy promień węzłów
        ...
        // Sprawdzamy rozmiar tekstu, aby wyśrodkować
        cairo_text_extents_t extents;
        cairo_text_extents(cr, string(1, root->dataItem).c_str(), &extents);
        double textWidth = extents.width;
        double textHeight = extents.height;

        // Wyśrodkowanie tekstu
        cairo_move_to(cr, rootX - textWidth / 2, rootY + textHeight / 2);
        cairo_show_text(cr, string(1, root->dataItem).c_str());
        ...
        function<void(Node*, double, double, double)> drawSubtree =
            [&](Node* n, double x, double y, double offset) {
                ...
                // Wyśrodkowanie tekstu
                cairo_text_extents(cr, string(1, n->dataItem).c_str(),
                                   &extents);
                ...
                cairo_move_to(cr, x - textWidthNode / 2,
                              y + textHeightNode / 2);
                cairo_show_text(cr, string(1, n->dataItem).c_str());
        ...
        drawSubtree(root, width / 2, 100, 180);  // Zwiększamy odległość
    }
